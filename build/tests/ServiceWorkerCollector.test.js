...
describe('ServiceWorkerConsoleCollector', () => {
    it('limits logs to 1000 per extension', () => {
        ...
        for (let i = 0; i < 15; i++) {
            const error = new UncaughtError({ ...mockDetails, exceptionId: i }, extensionId);
        ...
        assert.strictEqual(firstLog.details.exceptionId, 5, 'Oldest log should be Log 5');
        const lastLog = logs[logs.length - 1];
        assert.strictEqual(lastLog.details.exceptionId, 14, 'Last log should be Log 14');
        ...
        assert.strictEqual(foundLog, logToFind, 'getById should return correct log');
        const foundViaFind = collector.find(extensionId, item => {
            return item[stableIdSymbol] === logId;
        });
        assert.strictEqual(foundViaFind, logToFind, 'find should return correct log');
    });
});
//# sourceMappingURL=ServiceWorkerCollector.test.js.map