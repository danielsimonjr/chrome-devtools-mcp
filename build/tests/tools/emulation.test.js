    ...
    describe('cpu', () => {
        ...
        it('applies cpu throttling to secondary session', async () => {
            await withMcpContext(async (response, context) => {
                ...
                const cpuCall = sendSpy.mock.calls.find(call => call.arguments[0] === 'Emulation.setCPUThrottlingRate');
                assert.ok(cpuCall);
    ...
    describe('viewport', () => {
        ...
        it('reports correctly for the currently selected page', async () => {
            await withMcpContext(async (response, context) => {
                ...
                context.selectPage(page);
                assert.strictEqual(context.getSelectedMcpPage().viewport, null);
                assert.ok(await context.getSelectedMcpPage().pptrPage.evaluate(() => {
                    return window.innerWidth !== 400 && window.innerHeight !== 400;
                }));
            });
        });
    ...
    describe('extraHttpHeaders', () => {
        ...
        it('clears extra headers when null is passed', async () => {
            ...
            await withMcpContext(async (response, context) => {
                ...
                assert.strictEqual(context.getSelectedMcpPage().emulationSettings.extraHttpHeaders, undefined);
            });
        });
        ...
        it('reports correctly for the currently selected page', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.deepStrictEqual(context.getSelectedMcpPage().emulationSettings.extraHttpHeaders, { 'X-Page': 'one' });
                const page = await context.newPage();
                context.selectPage(page);
                assert.strictEqual(context.getSelectedMcpPage().emulationSettings.extraHttpHeaders, undefined);
            });
    ...
    describe('colorScheme', () => {
        ...
        it('updates color scheme', async () => {
            await withMcpContext(async (response, context) => {
                ...
                const page = context.getSelectedMcpPage().pptrPage;
                const scheme = await page.evaluate(() => window.matchMedia('(prefers-color-scheme: light)').matches
                    ? 'light'
                    : 'dark');
        ...
        it('resets color scheme when set to auto', async () => {
            await withMcpContext(async (response, context) => {
                const page = context.getSelectedMcpPage().pptrPage;
                const initial = await page.evaluate(() => window.matchMedia('(prefers-color-scheme: dark)').matches);
                await emulate.handler({
                    params: {
                        colorScheme: 'dark',
                    },
                ...
                assert.strictEqual(await page.evaluate(() => window.matchMedia('(prefers-color-scheme: dark)').matches), true);
                await emulate.handler({
                    params: {
                        colorScheme: 'auto',
                    },
                ...
                assert.strictEqual(await page.evaluate(() => window.matchMedia('(prefers-color-scheme: dark)').matches), initial);
            });
        });
    });
});
//# sourceMappingURL=emulation.test.js.map