...
import { installPwa, uninstallPwa, launchPwa, getOsAppState, } from '../../src/tools/pwa.js';
...
describe('pwa', () => {
    ...
    function setupPwaRoutes() {
        ...
        server.addHtmlRoute('/pwa/secondary.html', '<h1>Explicit PWA launch URL</h1>');
        return {
    ...
    it('installs a PWA, reads its OS state, and uninstalls it', async () => {
        ...
        await withMcpContext(async (response, context) => {
            await installPwa.handler({ params: { manifestId, installUrlOrBundleUrl: startUrl } }, response, context);
            assert.ok(response.responseLines.some(l => l.includes(manifestId)), 'install response should reference the manifest ID');
            response.resetResponseLineForTesting();
            await getOsAppState.handler({ params: { manifestId } }, response, context);
            const stateOutput = response.responseLines.join('\n');
            assert.ok(stateOutput.includes('Badge count: 0'), `expected a badge count in OS state, got: ${stateOutput}`);
            assert.ok(stateOutput.includes('File handlers: []'), `expected file handlers in OS state, got: ${stateOutput}`);
            const includePages = sinon.spy(response, 'setIncludePages');
            await uninstallPwa.handler({ params: { manifestId } }, response, context);
            assert.ok(includePages.calledOnce, 'uninstall should refresh the page list after closing app windows');
            // Querying OS state after uninstall should reject.
            await assert.rejects(getOsAppState.handler({ params: { manifestId } }, response, context));
        }, PWA_BROWSER_OPTIONS, { categoryPwa: true });
    });
    ...
    it('launches an installed PWA in a standalone window', async () => {
        ...
        await withMcpContext(async (response, context) => {
            ...
            response.resetResponseLineForTesting();
            await launchPwa.handler({ params: { manifestId } }, response, context);
            assert.ok(response.responseLines.some(l => l.includes(startUrl)), `launch response should reference the app url, got: ${response.responseLines.join('\n')}`);
            ...
            assert.strictEqual(isStandalone, true, 'the launched app should be in standalone display mode');
            await uninstallPwa.handler({ params: { manifestId } }, response, context);
        }, PWA_BROWSER_OPTIONS, { categoryPwa: true });
    });
    it('launches an installed PWA at an explicit URL', async () => {
        ...
        await withMcpContext(async (response, context) => {
            ...
            response.resetResponseLineForTesting();
            await launchPwa.handler({ params: { manifestId, url: explicitUrl } }, response, context);
            assert.ok(response.responseLines.some(line => {
    ...
    it('selects a surviving page after uninstall closes the selected app', async () => {
        ...
        await withMcpContext(async (response, context) => {
            ...
            assert.match(getTextContent(result.content[0]), /previously selected page was closed/);
        }, PWA_BROWSER_OPTIONS, { categoryPwa: true });
    });
});
//# sourceMappingURL=pwa.test.js.map