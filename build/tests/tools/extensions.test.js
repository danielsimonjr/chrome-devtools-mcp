...
import { installExtension, uninstallExtension, listExtensions, reloadExtension, triggerExtensionAction, } from '../../src/tools/extensions.js';
import { serverHooks } from '../server.js';
import { assertNoServiceWorkerReported, extractExtensionId, withMcpContext, html, getTextContent, } from '../utils.js';
const EXTENSION_WITH_SW_PATH = path.join(import.meta.dirname, '../../../tests/tools/fixtures/extension-sw');
const EXTENSION_PATH = path.join(import.meta.dirname, '../../../tests/tools/fixtures/extension');
const EXTENSION_CONTENT_SCRIPT_PATH = path.join(import.meta.dirname, '../../../tests/tools/fixtures/extension-content-script');
describe('extension', () => {
    ...
    it('reloads an extension', async () => {
        await withMcpContext(async (response, context) => {
            ...
            await reloadExtension.handler({ params: { id: extensionId } }, response, context);
            assert.ok(installSpy.calledOnceWithExactly(EXTENSION_PATH), 'installExtension should be called with the extension path');
            const reloadResponseLine = response.responseLines[0];
            assert.ok(reloadResponseLine.includes('Extension reloaded'), 'Response should indicate reload');
    ...
    it('triggers an extension action', async () => {
        await withMcpContext(async (response, context) => {
            const extensionId = await context.installExtension(EXTENSION_WITH_SW_PATH);
            const targetsBefore = context.browser.targets();
            const pageTargetBefore = targetsBefore.find(t => t.type() === 'page' && t.url().includes(extensionId));
            assert.ok(!pageTargetBefore, 'Page should not exist before action');
            await triggerExtensionAction.handler({ params: { id: extensionId } }, response, context);
            const pageTargetAfter = await context.browser.waitForTarget(t => t.type() === 'page' && t.url().includes(extensionId));
    ...
    it('verifies that content script console logs are received', async () => {
        await withMcpContext(async (response, context) => {
            server.addHtmlRoute('/test-content-script', html `<h1>Test Content Script</h1>`);
            const url = server.getRoute('/test-content-script');
            const extensionId = await context.installExtension(EXTENSION_CONTENT_SCRIPT_PATH);
