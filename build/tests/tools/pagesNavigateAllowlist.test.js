...
describe('pages allowList', () => {
    ...
    const args = { experimentalNavigationAllowlist: true };
    it('navigates through redirects when all URLs are allowed', async () => {
        ...
        server.addHtmlRoute('/c.html', '<html><body><h1>Final Destination</h1></body></html>');
        await withMcpContext(async (response, context) => {
            ...
            const content = await page.pptrPage.evaluate(() => document.querySelector('h1')?.textContent);
    ...
    it('blocks navigation when a redirect target is not allowed', async () => {
        ...
        await withMcpContext(async (response, context) => {
            ...
            assert.ok(response.responseLines.some(line => line.includes('Unable to navigate')));
        });
    });
});
//# sourceMappingURL=pagesNavigateAllowlist.test.js.map