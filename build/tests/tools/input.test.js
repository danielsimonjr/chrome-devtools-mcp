...
import { click, hover, fill, drag, fillForm, uploadFile, pressKey, clickAt, typeText, } from '../../src/tools/input.js';
...
describe('input', () => {
    ...
    describe('click', () => {
        ...
        it('reports the new URL when click triggers a navigation', async () => {
            server.addHtmlRoute('/start', html `<a href="/after-click">Navigate page</a>`);
        ...
        it('selects a collapsed native select option by option uid', async () => {
            await withMcpContext(async (response, context) => {
                const page = context.getSelectedMcpPage().pptrPage;
                await page.setContent(html `<select onchange="document.body.dataset.selected = this.value">
            <option value="v1">one</option>
            <option value="v2">two</option>
          </select>`);
                const mcpPage = context.getSelectedMcpPage();
                mcpPage.textSnapshot = await TextSnapshot.create(mcpPage);
                const optionNode = [...mcpPage.textSnapshot.idToNode.values()].find(node => node.role === 'option' && node.name === 'two');
        ...
        it('clicks custom ARIA option elements through the normal click path', async () => {
            await withMcpContext(async (response, context) => {
                const page = context.getSelectedMcpPage().pptrPage;
                await page.setContent(html `<div role="listbox">
            <div
              role="option"
              tabindex="0"
            ...
            >
              custom two
                ...
                const mcpPage = context.getSelectedMcpPage();
                mcpPage.textSnapshot = await TextSnapshot.create(mcpPage);
                const optionNode = [...mcpPage.textSnapshot.idToNode.values()].find(node => node.role === 'option' && node.name === 'custom two');
                assert.ok(optionNode);
                await click.handler({
                    params: {
                        uid: optionNode.id,
                ...
                assert.strictEqual(response.responseLines[0], 'Successfully clicked on the element');
                assert.strictEqual(await page.evaluate(() => document.body.dataset.clicked), 'custom two');
            });
        });
    });
    describe('hover', () => {
    ...
    describe('fill', () => {
        ...
        it('types text with submit key', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.strictEqual(response.responseLines[0], 'Typed text "test + Tab"');
                assert.strictEqual(await page.evaluate(() => {
                    return document.body.querySelector('textarea')?.value;
                }), 'test');
                assert.ok(await page.evaluate(() => {
                    return (document.body.querySelector('textarea') !== document.activeElement);
                }));
            });
        });
        ...
        it('reproduction: fill isolation', async () => {
            ...
            />
                ...
                assert.strictEqual(response1.responseLines[0], 'Successfully filled out the element');
                // Fill password
                ...
                assert.strictEqual(response2.responseLines[0], 'Successfully filled out the element');
                // Verify values
        ...
        it('toggles checkboxes', async () => {
          ...
          />`);
                ...
                assert.strictEqual(response.responseLines[0], 'Successfully filled out the element');
                assert.ok(response.includeSnapshot);
                let isChecked = await page.$eval('#cb', el => el.checked);
                ...
                await fill.handler({
                    ...
                    page: context.getSelectedMcpPage(),
                }, new McpResponse({}), context);
                isChecked = await page.$eval('#cb', el => el.checked);
        ...
        `);
                ...
                let swChecked = await page.$eval('#sw', el => el.getAttribute('aria-checked') === 'true');
                ...
                swChecked = await page.$eval('#sw', el => el.getAttribute('aria-checked') === 'true');
        ...
        `);
                context.getSelectedMcpPage().textSnapshot = await TextSnapshot.create(context.getSelectedMcpPage());
                // Initial state
                let r1Checked = await page.$eval('#r1', el => el.checked);
                let r2Checked = await page.$eval('#r2', el => el.checked);
                ...
                r1Checked = await page.$eval('#r1', el => el.checked);
                r2Checked = await page.$eval('#r2', el => el.checked);
    ...
    describe('uploadFile', () => {
        ...
        it('uploads multiple files to a file input', async () => {
            ...
            />
                ...
                assert.strictEqual(response.responseLines[0], `File uploaded from ${firstFilePath}, ${secondFilePath}.`);
    ...
    describe('press_key', () => {
        ...
        it('releases held modifiers when the main key press fails', async () => {
            ...
            document.addEventListener('keyup', e => logs.push('u' + e.key));
                ...
                try {
                    await assert.rejects(pressKey.handler({
                        params: {
                            key: 'Control+Shift+C',
                        },
                        page: context.getSelectedMcpPage(),
