...
describe('console', () => {
    ...
    it('captures logs and errors from extension service worker', async (t) => {
        await withMcpContext(async (response, context) => {
            await installExtension.handler({ params: { path: EXTENSION_LOGGING_PATH } }, response, context);
            const extensionId = extractExtensionId(response);
            assert.ok(extensionId, 'Extension ID should be returned');
            const swTarget = await context.browser.waitForTarget(t => t.type() === 'service_worker' && t.url().includes(extensionId));
            ...
            const errorPromise = new Promise((resolve, reject) => {
                ...
                worker.client.on('Runtime.exceptionThrown', () => {
            ...
            const sanitizedText = textContent.replaceAll(new RegExp(extensionId, 'g'), '<extension-id>');
            t.assert.snapshot?.(sanitizedText);
            assert.ok(sanitizedText.includes('Service Worker starting...'), 'Should contain start log');
            assert.ok(sanitizedText.includes('This is a warning from Service Worker'), 'Should contain warning log');
            assert.ok(sanitizedText.includes('Intentional error from Service Worker'), 'Should contain error log');
    ...
    describe('list_console_messages', () => {
        ...
        it('lists error objects', async (t) => {
            await withMcpContext(async (response, context) => {
                const page = context.getSelectedMcpPage();
                await page.pptrPage.setContent('<script>console.error(new Error("This is an error"))</script>');
                await listConsoleMessages().handler({ params: {}, page: context.getSelectedMcpPage() }, response, context);
                const formattedResponse = await response.handle('test', context);
                const textContent = getTextContent(formattedResponse.content[0]);
    ...
    describe('get_console_message', () => {
        ...
        describe('issues type', () => {
            it('gets issue details with node id parsing', async (t) => {
                await withMcpContext(async (response, context) => {
                    ...
                    await getConsoleMessage.handler({ params: { msgid: 1 }, page: context.getSelectedMcpPage() }, response2, context);
                    const formattedResponse = await response2.handle('test', context);
        ...
        it('ignores frames from ignore listed URLs', async (t) => {
            server.addHtmlRoute('/index.html', `<!DOCTYPE html>
         <script>
