...
import { assertNoServiceWorkerReported, extractExtensionId, getTextContent, html, withMcpContext, } from '../utils.js';
    ...
    describe('browser_evaluate_script', () => {
        ...
        it('still awaits a navigation when waitForStableDom is false', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.ok(textContent.includes(`Page navigated to ${url}`), `Expected the navigation to be awaited and reported, got: ${textContent}`);
            });
        });
        ...
        it('saves output to file when filePath is provided', async () => {
            ...
            try {
                await withMcpContext(async (response, context) => {
                    ...
                    assert.ok(response.responseLines[0]?.includes('Output saved to'), `Expected "Output saved to" but got: ${response.responseLines[0]}`);
                });
        ...
        it('throws error when both pageId and serviceWorkerId are provided', async () => {
            await withMcpContext(async (response, context) => {
                await assert.rejects(evaluateScript({
                    categoryExtensions: true,
                }).handler({
                    params: {
