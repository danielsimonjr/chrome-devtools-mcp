...
const EXTENSION_SW_PATH = path.join(import.meta.dirname, '../../../tests/tools/fixtures/extension-sw');
const EXTENSION_PATH = path.join(import.meta.dirname, '../../../tests/tools/fixtures/extension');
const EXTENSION_SIDE_PANEL_PATH = path.join(import.meta.dirname, '../../../tests/tools/fixtures/extension-side-panel');
describe('pages', () => {
    afterEach(() => {
        sinon.restore();
    });
    describe('list_pages', () => {
        ...
        it(`list pages for extension pages with --category-extensions`, async (t) => {
            await withMcpContext(async (response, context) => {
                ...
                const _popupTarget = await context.browser.waitForTarget(t => t.type() === 'page' && t.url().includes('chrome-extension://'));
                ...
                const text = textContent.text.replaceAll(extensionId, '<extension-id>');
        ...
        for (const categoryExtensions of [true, false]) {
            it(`list pages for extension service workers ${categoryExtensions ? 'with' : 'without'} --category-extensions`, async (t) => {
                await withMcpContext(async (response, context) => {
                    ...
                    assert.ok(extensionId);
                    const swTarget = await context.browser.waitForTarget(target => target.type() === 'service_worker' &&
                    ...
                    if (categoryExtensions) {
                        ...
                        assert.deepStrictEqual(structured.extensionServiceWorkers.map(sw => sw.url), [swUrl]);
                    }
                    const text = textContent.text.replaceAll(extensionId, '<extension-id>');
                    t.assert.snapshot(text);
                    await context.uninstallExtension(extensionId);
        ...
        it('list pages for side panels with --category-extensions', async (t) => {
            await withMcpContext(async (response, context) => {
                const extensionId = await context.installExtension(EXTENSION_SIDE_PANEL_PATH);
                assert.ok(extensionId);
                const sidePanelPage = await context.newPage();
                await sidePanelPage.pptrPage.goto(`chrome-extension://${extensionId}/sidepanel.html`);
                await context.waitForTextOnPage(['Side Panel']);
                // Wait for service worker used in the snapshot.
                await context.browser.waitForTarget(target => target.type() === 'service_worker');
                const listPageDef = listPages({
                    categoryExtensions: true,
                });
                await listPageDef.handler({ params: {} }, response, context);
    ...
    describe('new_page', () => {
        it('create a page', async () => {
            await withMcpContext(async (response, context) => {
                assert.strictEqual(context.getPageById(1), context.getSelectedMcpPage());
                await newPage().handler({ params: { url: 'about:blank' } }, response, context);
                assert.strictEqual(context.getPageById(2), context.getSelectedMcpPage());
                assert.ok(response.includePages);
            });
        });
        it('create a page in the background', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.strictEqual(await originalPage.pptrPage.evaluate(() => document.hasFocus()), true);
    ...
    describe('new_page with isolatedContext', () => {
        it('creates a page in an isolated context', async () => {
            await withMcpContext(async (response, context) => {
                await newPage().handler({ params: { url: 'about:blank', isolatedContext: 'session-a' } }, response, context);
                const page = context.getSelectedPptrPage();
        ...
        it('creates separate contexts for different isolatedContext names', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await newPage().handler({ params: { url: 'about:blank', isolatedContext: 'session-b' } }, response, context);
        ...
        it('does not set isolatedContext for pages in the default context', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await newPage().handler({ params: { url: 'about:blank' } }, response, context);
                assert.strictEqual(context.getIsolatedContextName(context.getSelectedPptrPage()), undefined);
            });
        });
    ...
    it('navigate_page targets the pageId page, not the global selection', async () => {
        await withMcpContext(async (response, context) => {
            ...
            const content = await isolatedPage.pptrPage.evaluate(() => document.querySelector('h1')?.textContent);
    ...
    describe('select_page', () => {
        ...
        it('selects a page and keeps it focused in the background', async () => {
            await withMcpContext(async (response, context) => {
                await context.newPage();
                assert.strictEqual(context.getPageById(2), context.getSelectedMcpPage());
                assert.strictEqual(await context
                    .getPageById(1)
                    .pptrPage.evaluate(() => document.hasFocus()), true);
                await selectPage.handler({ params: { pageId: 1 } }, response, context);
                assert.strictEqual(context.getPageById(1), context.getSelectedMcpPage());
                assert.strictEqual(await context
                    .getPageById(1)
        ...
        it('preserves focus across different browser contexts', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await newPage().handler({ params: { url: 'about:blank', isolatedContext: 'ctx-a' } }, response, context);
                const pageA = context.getSelectedPptrPage();
                const pageAId = context.getPageId(pageA);
                await newPage().handler({ params: { url: 'about:blank', isolatedContext: 'ctx-b' } }, response, context);
                const pageB = context.getSelectedPptrPage();
                // Selecting pageB (ctx-b) should not defocus pageA (ctx-a).
                assert.strictEqual(await pageA.evaluate(() => document.hasFocus()), true);
                assert.strictEqual(await pageB.evaluate(() => document.hasFocus()), true);
                // Switching back to pageA should preserve pageB's focus.
                await selectPage.handler({ params: { pageId: pageAId } }, response, context);
                assert.strictEqual(await pageA.evaluate(() => document.hasFocus()), true);
                assert.strictEqual(await pageB.evaluate(() => document.hasFocus()), true);
            });
        });
    ...
    describe('navigate_page', () => {
        ...
        it('respects the timeout parameter', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.strictEqual(stub.firstCall.args[0]?.timeout, 12345, 'The timeout parameter should be passed to waitForNavigation');
            });
        });
        it('go back', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await navigatePage().handler({ params: { type: 'back' }, page: context.getSelectedMcpPage() }, response, context);
        ...
        it('go forward', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await page.goto('data:text/html,<div>Hello MCP</div>');
                await page.goBack();
                await navigatePage().handler({ params: { type: 'forward' }, page: context.getSelectedMcpPage() }, response, context);
                assert.equal(await page.evaluate(() => document.querySelector('div')?.textContent), 'Hello MCP');
                assert.ok(response.includePages);
            });
        });
        it('reload', async () => {
            await withMcpContext(async (response, context) => {
                const page = context.getSelectedPptrPage();
                await page.goto('data:text/html,<div>Hello MCP</div>');
                await navigatePage().handler({ params: { type: 'reload' }, page: context.getSelectedMcpPage() }, response, context);
    ...
    describe('dialogs', () => {
        ...
        it('tracks dialogs independently per page', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await handleDialog.handler({ params: { action: 'accept' }, page: page1 }, response, context);
                assert.strictEqual(page1.getDialog(), undefined);
                assert.ok(page2.getDialog());
                // Handle page2's dialog.
                await handleDialog.handler({ params: { action: 'dismiss' }, page: page2 }, response, context);
