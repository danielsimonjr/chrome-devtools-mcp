    ...
    describe('browser_snapshot', () => {
        it('includes a snapshot', async () => {
            await withMcpContext(async (response, context) => {
                await takeSnapshot.handler({ params: {}, page: context.getSelectedMcpPage() }, response, context);
    ...
    describe('browser_wait_for', () => {
        ...
        it('should work with any-match array', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.equal(response.responseLines[0], 'Element matching one of ["Complete","Error"] found.');
                assert.ok(response.includeSnapshot);
            });
        });
        it('should work with any-match array when element shows up later', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await page.setContent(html `<main
