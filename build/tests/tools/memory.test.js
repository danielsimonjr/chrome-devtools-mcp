...
describe('memory', () => {
    ...
    describe('get_heapsnapshot_class_nodes', () => {
        ...
        it('with objectsRetainedByContexts filterName', async (t) => {
            await withMcpContext(async (response, context) => {
                const filePath = join(process.cwd(), 'tests/fixtures/example.heapsnapshot');
                const aggregateData = await context.getHeapSnapshotAggregates(filePath, 'objectsRetainedByContexts');
                const aggregate = Object.values(aggregateData.aggregates).find(a => a.name === 'Function');
                assert.ok(aggregate);
        ...
        it('with non-existent class name', async () => {
            await withMcpContext(async (response, context) => {
                const filePath = join(process.cwd(), 'tests/fixtures/example.heapsnapshot');
                await context.getHeapSnapshotAggregates(filePath);
                await assert.rejects(getHeapSnapshotClassNodes.handler({ params: { filePath, id: 999999 } }, response, context), { message: 'Class with ID 999999 not found in heap snapshot' });
            });
        });
    });
    ...
    describe('close_heapsnapshot', () => {
        it('with default options', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await closeHeapSnapshot.handler({ params: { filePath } }, response, context);
                assert.ok(response.responseLines.includes(`Closed heap snapshot: ${filePath}`));
                assert.ok(!context.hasHeapSnapshots());
            });
        });
        it('with non-existent snapshot', async () => {
            await withMcpContext(async (response, context) => {
                const filePath = join(process.cwd(), 'tests/fixtures/example.heapsnapshot');
                await assert.rejects(closeHeapSnapshot.handler({ params: { filePath } }, response, context), {
    ...
    describe('get_heapsnapshot_retaining_paths', () => {
        ...
        it('reports when limits are reached', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.match(output, /Note: results are truncated, the following limits were reached: depth\./);
            });
        });
    });
    ...
    describe('compare_heapsnapshots', () => {
        ...
        it('compare heap-1 to heap-2 with invalid classIndex throws error', async () => {
            await withMcpContext(async (response, context) => {
                const filePathA = join(process.cwd(), 'tests/fixtures/heap-1.heapsnapshot');
                const filePathB = join(process.cwd(), 'tests/fixtures/heap-2.heapsnapshot');
                await assert.rejects(compareHeapSnapshots.handler({
                    params: {
                        baseFilePath: filePathA,
                        currentFilePath: filePathB,
                        classIndex: 99,
                    },
                }, response, context), /Invalid classIndex: 99. Total classes with changes: 10/);
            });
        });
    });
    ...
    it('compares the same current snapshot against different bases', async () => {
        await withMcpContext(async (_response, context) => {
            const filePathA = join(process.cwd(), 'tests/fixtures/heap-1.heapsnapshot');
            const filePathB = join(process.cwd(), 'tests/fixtures/heap-2.heapsnapshot');
            const filePathC = join(process.cwd(), 'tests/fixtures/heap-3.heapsnapshot');
            const firstDiff = await context.getHeapSnapshotClassDiffs(filePathA, filePathC);
            const secondDiff = await context.getHeapSnapshotClassDiffs(filePathB, filePathC);
            const firstNewObjectDiff = firstDiff.find(entry => entry.className === 'NewObject');
            const secondNewObjectDiff = secondDiff.find(entry => entry.className === 'NewObject');
