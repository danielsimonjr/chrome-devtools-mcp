...
describe('webmcp', () => {
    ...
    describe('execute_webmcp_tool', () => {
        async function setupWebMcpTool(page) {
            await page.pptrPage.setContent(html `<form
            toolname="test_tool"
        ...
        it('throws if input is invalid', async () => {
            await withMcpContext(async (response, context) => {
                await assert.rejects(async () => {
                    const page = context.getSelectedMcpPage();
                    await setupWebMcpTool(page);
                    await executeWebMcpTool.handler({ params: { toolName: 'test_tool', input: 'invalid' }, page }, response, context);
                }, {
                    message: /Failed to parse input as JSON: Unexpected token 'i', "invalid" is not valid JSON/,
                });
            }, { args: ['--enable-features=WebMCP,DevToolsWebMCPSupport'] }, { categoryExperimentalWebmcp: true });
        });
    });
});
//# sourceMappingURL=webmcp.test.js.map