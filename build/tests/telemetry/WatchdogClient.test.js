...
describe('WatchdogClient', () => {
    ...
    it('spawns watchdog process with correct arguments', () => {
        ...
        assert.match(cmdArgs[0], /watchdog[/\\]main\.js$/, 'First argument should be path to watchdog/main.js');
        assert.ok(cmdArgs.includes('--parent-pid=100'), 'Arguments should include parent PID');
        assert.ok(cmdArgs.includes('--app-version=1.2.3'), 'Arguments should include app version');
        assert.ok(cmdArgs.includes('--os-type=2'), 'Arguments should include OS type');
        assert.strictEqual(spawnStub.firstCall.args[2].detached, true, 'Process should be spawned as detached');
    });
    it('passes log-file argument if provided', () => {
        ...
        assert.ok(cmdArgs.includes('--log-file=/tmp/test.log'), 'Arguments should include log file path');
    });
    it('sends IPC messages via stdin', () => {
        ...
        client.send(msg);
        assert.ok(stdinStub.write.calledOnce, 'Expected `stdin.write` to be called');
        const writtenData = stdinStub.write.firstCall.args[0];
        assert.strictEqual(writtenData.trim(), JSON.stringify(msg), 'Written data should match expected JSON message');
    });
...
//# sourceMappingURL=WatchdogClient.test.js.map