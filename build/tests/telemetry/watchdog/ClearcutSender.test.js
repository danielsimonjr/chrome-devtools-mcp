...
describe('ClearcutSender', () => {
    ...
    it('correctly handles buffer overflow during queued flush', async () => {
        ...
        let resolveRequest;
        fetchStub.onFirstCall().returns(new Promise(resolve => {
    ...
    it('does not duplicate events when shutdown occurs during an active flush', async () => {
        ...
        const firstBody = JSON.parse(fetchStub.args[0][1].body);
        const secondBody = JSON.parse(fetchStub.args[1][1].body);
        ...
        assert.strictEqual(secondEvents.length, 1, 'Shutdown request should only contain shutdown event');
    ...
    it('rotates session id after 24 hours', async () => {
        ...
        await clock.tickAsync(FLUSH_INTERVAL_MS);
        const firstCallBody = JSON.parse(fetchStub.firstCall.args[1].body);
        ...
        await clock.tickAsync(FLUSH_INTERVAL_MS);
        const secondCallBody = JSON.parse(fetchStub.secondCall.args[1].body);
...
//# sourceMappingURL=ClearcutSender.test.js.map