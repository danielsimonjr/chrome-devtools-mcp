...
describe('sanitizeParams', () => {
    ...
    it('throws error for unsupported types', () => {
        ...
        assert.throws(() => sanitizeParams(params, schema), /Unsupported zod type for tool parameter: ZodObject/);
    });
    it('throws error when value is not of equivalent type', () => {
        ...
        assert.throws(() => sanitizeParams(params, schema), /parameter myString has type ZodString but value 123 is not of equivalent type/);
    });
});
describe('stripUnderscoreBeforeNumber', () => {
    it('removes underscores immediately preceding numbers', () => {
        assert.strictEqual(stripUnderscoreBeforeNumber('list_3p_developer_tools'), 'list3p_developer_tools');
        assert.strictEqual(stripUnderscoreBeforeNumber('make_2g_network_request'), 'make2g_network_request');
        assert.strictEqual(stripUnderscoreBeforeNumber('no_numbers_here'), 'no_numbers_here');
    });
});
