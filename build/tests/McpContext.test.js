...
import { getMockRequest, html, withBrowser, withMcpContext, stabilizeStructuredContent, } from './utils.js';
describe('McpContext', () => {
    afterEach(() => {
        sinon.restore();
    });
    ...
    it('should should detect open DevTools pages', async () => {
        await withMcpContext(async (_response, context) => {
            ...
            assert.ok(listed.every(mcpPage => !mcpPage.pptrPage.url().startsWith('devtools://')), 'listing should exclude devtools pages');
    ...
    it('keeps a still-open selected page that is missing from the list', async () => {
        await withMcpContext(async (_response, context) => {
            ...
            assert.ok(context.isPageSelected(page), 'a still-open page should keep its selection');
            assert.strictEqual(context.getSelectedPageFallback(), undefined);
        });
    });
    it('continues page ids across contexts so stale ids do not resolve', async () => {
        await withBrowser(async (browser) => {
            ...
            const second = await McpContext.from(browser, undefined, options, Locator);
            try {
                assert.throws(() => second.getPageById(idBeforeReconnect), /No page found/);
                assert.ok((await second.newPage()).id > idBeforeReconnect, 'ids continue past the pre-reconnect ids');
            }
            finally {
                second.dispose();
            }
    ...
    it('reports the reconnect notice once', async () => {
        await withBrowser(async (browser) => {
            const context = await McpContext.from(browser, undefined, {
                experimentalDevToolsDebugging: false,
                performanceCrux: false,
                reconnected: true,
            }, Locator);
    ...
    it('disposes loaded heap snapshots on teardown', async () => {
        await withMcpContext(async (_response, context) => {
            const filePath = path.join(process.cwd(), 'tests/fixtures/example.heapsnapshot');
            await context.getHeapSnapshotStats(filePath);
            assert.ok(context.hasHeapSnapshots(), 'snapshot loaded before teardown');
            context.dispose();
            assert.ok(!context.hasHeapSnapshots(), 'heap snapshots freed on teardown');
        });
    });
    ...
    it('can store and retrieve roots', async () => {
        await withMcpContext(async (_response, context) => {
            ...
            assert.ok(actualRoots?.some(r => r.name === 'test'), 'Should contain the set root');
            assert.ok(actualRoots?.some(r => r.name === 'temp'), 'Should contain the temp root');
        });
    });
    it('validatePath allows paths within roots', async () => {
        await withMcpContext(async (_response, context) => {
            ...
            try {
                ...
                assert.strictEqual(resolvedWorkspace, await resolveCanonicalPath(workspacePath));
    ...
    it('validatePath returns resolved absolute path for relative paths', async () => {
        await withMcpContext(async (_response, context) => {
            ...
            const relativeTmpPath = path.relative(process.cwd(), path.join(tmpDir, 'test.txt'));
    ...
    it('validatePath denies paths outside os.tmpdir() if roots list is empty', async () => {
        await withMcpContext(async (_response, context) => {
            ...
            await assert.rejects(context.validatePath(path.resolve(os.homedir(), 'anywhere.txt')), /Access denied/);
        });
    });
    describe('symlink security checks', () => {
        ...
        it('validatePath resolves symlinks and returns the canonical path', async () => {
            await withMcpContext(async (_response, context) => {
                const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'validate-symlink-test-'));
                try {
        ...
        it('saveFile refuses to write through a dangling symlink to a non-existent file', async () => {
            await withMcpContext(async (_response, context) => {
                const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mcp-symlink-test-'));
                const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mcp-outside-test-'));
                try {
                    context.setRoots([{ uri: pathToFileURL(tmpDir).href, name: 'temp' }]);
                    ...
                    await assert.rejects(context.saveFile(data, symlinkPath, '.txt'), /Could not write/);
        ...
        it('saveFile allows writing to a file within an allowed symlinked directory', async () => {
            await withMcpContext(async (_response, context) => {
                ...
                try {
                    ...
                    assert.strictEqual(result.filename, await resolveCanonicalPath(targetFilePath));
                    const content = await fs.readFile(path.join(realDir, 'test.txt'), 'utf-8');
    ...
    describe('loadResource', () => {
        describe('file protocol', () => {
            ...
            it('is not blocked by allowlist', async () => {
                const testFilePath = path.join(os.tmpdir(), 'load-resource-test-allow.txt');
                await fs.writeFile(testFilePath, 'test content');
                try {
                    await withMcpContext(async (_response, context) => {
                        const url = pathToFileURL(testFilePath).href;
        ...
        describe('https protocol', () => {
            it('respects blocklist by throwing if blocked', async () => {
                await withMcpContext(async (_response, context) => {
                    await assert.rejects(() => context.loadResource('https://example.com/blocked'));
                }, {
            ...
            it('respects blocklist by not throwing if not blocked', async () => {
                await withMcpContext(async (_response, context) => {
                    ...
                    const content = await context.loadResource('https://example.com/allowed');
        ...
        describe('getDevToolsData', () => {
            ...
            it('returns undefined when getDevToolsData times out', async () => {
                await withMcpContext(async (_response, context) => {
                    const mockPage = sinon.createStubInstance(McpPage);
                    mockPage.getDevToolsData.returns(new Promise(resolve => {
...
//# sourceMappingURL=McpContext.test.js.map