...
import { createTargetUniverse, SymbolizedError, } from '../../src/devtools/DevtoolsUtils.js';
...
describe('createTargetUniverse', () => {
    ...
    it('works with a real browser', async () => {
        await withBrowser(async (browser, page) => {
            const targetUniverse = await createTargetUniverse(await page.createCDPSession());
    ...
    it('ignores pauses', async () => {
        await withBrowser(async (browser, page) => {
            ...
            model.addEventListener('DebuggerPaused', pausedSpy); // eslint-disable-line
    ...
    it('ignores pauses during navigation', async () => {
        server.addHtmlRoute('/debugger', html `<script>
          debugger;
        </script>
        <h1>Loaded</h1>`);
        await withBrowser(async (browser, page) => {
    ...
    it('disables network domain', async () => {
        ...
        await withBrowser(async (browser, page) => {
            const targetUniverse = await createTargetUniverse(await page.createCDPSession());
            assert.ok(targetUniverse);
            const networkManager = targetUniverse.target.model(DevTools.NetworkManager.NetworkManager);
            assert.ok(networkManager);
            const requestStartedSpy = sinon.stub();
            networkManager.addEventListener(DevTools.NetworkManager.Events.RequestStarted, requestStartedSpy);
...
describe('SymbolizedError', () => {
    function exceptionDetails(overrides = {}) {
        return {
            exceptionId: 1,
            text: 'Uncaught',
            lineNumber: 0,
            ...
            const error = await SymbolizedError.fromDetails({
                details: exceptionDetails({
                    exception: {
                        ...
                        subtype: 'error',
                        className: 'Error',
                        description: 'Error: first line\nsecond line\n    at foo (http://example.com/app.js:1:1)',
                    },
                }),
                targetId: '',
            });
            assert.strictEqual(error.message, 'Uncaught Error: first line\nsecond line');
        });
...
//# sourceMappingURL=DevtoolsUtils.test.js.map