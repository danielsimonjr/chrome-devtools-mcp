...
import { ConsoleCollector, NetworkCollector, PageCollector, } from '../../src/collectors/PageCollector.js';
...
describe('NetworkCollector', () => {
    it('retains only the newest requests without navigation', async () => {
        ...
        }, (_, index) => getMockRequest({ url: `http://example.com/request-${index + 1}` }));
        for (const request of requests) {
            page.emit('request', request);
        }
        const retainedRequests = collector.getData();
        assert.equal(retainedRequests.length, NetworkCollector.MAX_REQUESTS_PER_NAVIGATION);
...
describe('ConsoleCollector', () => {
    ...
    it('emits UncaughtErrors for Runtime.exceptionThrown CDP events', async () => {
        ...
        sinon.assert.calledOnceWithMatch(onUncaughtErrorListener, sinon.match(e => {
            return (e.details.exception.description === 'SyntaxError: Expected {',
                e.details.text === 'Uncaught',
                e.details.stackTrace.callFrames.length === 0);
        }));
    });
});
//# sourceMappingURL=PageCollector.test.js.map