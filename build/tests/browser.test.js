...
async function runWithRetry(fn) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            await Promise.race([
                fn(),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Test execution timeout exceeded')), 20000)),
            ]);
            return;
        }
        catch (e) {
        ...
        await runWithRetry(async () => {
            ...
            try {
                try {
                    ...
                    assert.strictEqual(err.message, `The browser is already running for ${folderPath}. Use --isolated to run multiple browser instances.`);
                }
            }
            finally {
