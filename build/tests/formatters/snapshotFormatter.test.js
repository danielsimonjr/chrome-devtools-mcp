...
describe('snapshotFormatter', () => {
    it('formats a snapshot with value properties', () => {
        ...
        assert.strictEqual(formatted, `uid=1_1 textbox "textbox" details="details-id" errormessage="error-id" live="polite" relevant="additions" value="value"
...
`);
    ...
    it('formats a snapshot with boolean properties', () => {
        ...
        const formatter = new SnapshotFormatter({ root: node });
        const formatted = formatter.toString();
        assert.strictEqual(formatted, `uid=1_1 button "button" atomic busy disableable disabled
  uid=1_2 statictext "text"
`);
    });
