...
import { ConsoleFormatter, GroupedConsoleFormatter, } from '../../src/formatters/ConsoleFormatter.js';
const createMockMessage = (type, text, argsCount = 0) => {
...
const makeFormatter = (id, type, text, argsCount = 0) => ConsoleFormatter.from(createMockMessage(type, text, argsCount), { id });
describe('ConsoleFormatter grouping', () => {
    ...
    describe('GroupedConsoleFormatter output', () => {
        ...
        it('toJSON includes count field', async () => {
            ...
            const grouped = ConsoleFormatter.groupConsecutive(msgs);
            assert.strictEqual(grouped.length, 1);
            const json = grouped[0].toJSON();
            assert.strictEqual(json.count, 3);
            assert.strictEqual(json.id, 1);
        });
    });
});
//# sourceMappingURL=ConsoleFormatterGrouping.test.js.map