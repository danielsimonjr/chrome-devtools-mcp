...
describe('IssueFormatter', () => {
    ...
    function getMockIssueWithDetails(details) {
    ...
    formatterTestDetailed('formats a detailed issue with a resolved request id', async () => {
        ...
        return new IssueFormatter(mockAggregatedIssue, {
            id: 6,
            requestIdResolver: requestId => requestId === 'REQUEST-1' ? 42 : undefined,
        });
    });
    formatterTestDetailed('formats a detailed issue with an unresolved request id', async () => {
        const mockAggregatedIssue = getMockIssueWithDetails({
            request: {
                url: 'http://example.com/data.json',
                requestId: 'REQUEST-1',
    ...
    it('falls back to "Unknown Issue" when there is no description metadata', () => {
        ...
        assert.strictEqual(formatter.toString(), 'msgid=3 [issue] Unknown Issue (count: 1)');
        assert.strictEqual(formatter.toStringDetailed(), 'ID: 3\nMessage: issue> Unknown Issue');
    });
    describe('affected resources', () => {
        it('resolves nodeId with the element id resolver', () => {
            const formatter = new IssueFormatter(getMockIssueWithDetails({ nodeId: 42, extra: 'info' }), {
                id: 1,
                elementIdResolver: backendNodeId => backendNodeId === 42 ? '2_7' : undefined,
            });
        ...
        it('resolves documentNodeId with the element id resolver', () => {
            const formatter = new IssueFormatter(getMockIssueWithDetails({ documentNodeId: 7 }), {
                id: 1,
                elementIdResolver: backendNodeId => backendNodeId === 7 ? '3_1' : undefined,
            });
            assert.deepStrictEqual(formatter.toJSONDetailed().affectedResources, [
        ...
        it('keeps node ids if there is no element id resolver', () => {
            const formatter = new IssueFormatter(getMockIssueWithDetails({ nodeId: 42 }), { id: 1 });
            assert.deepStrictEqual(formatter.toJSONDetailed().affectedResources, [
...
//# sourceMappingURL=IssueFormatter.test.js.map