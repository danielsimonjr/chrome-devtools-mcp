...
describe('ToolHandler', () => {
    ...
    it('calls getPageById for page scoped tools when pageId is provided', async () => {
        ...
        const toolHandler = new ToolHandler(tool, serverArgs, async () => mockContext, toolMutex);
    ...
    it('calls getSelectedMcpPage for page scoped tools when pageIdRouting is disabled', async () => {
        ...
        const toolMutex = new Mutex();
        const serverArgs = parseArguments('1.0.0', ['node', 'script.js', '--no-page-id-routing'], { CHROME_DEVTOOLS_MCP_NO_USAGE_STATISTICS: 'true' });
        const toolHandler = new ToolHandler(tool, serverArgs, async () => mockContext, toolMutex);
        assert.strictEqual(toolHandler.shouldRegister, true);
    ...
    it('appends correct context to tool call logs', async () => {
        ...
        for (const testCase of testCases) {
            ...
            const toolHandler = new ToolHandler(testCase.tool, serverArgs, async () => mockContext, toolMutex);
            await toolHandler.handle({});
            assert.strictEqual(logSpy.calledOnce, true);
            assert.deepStrictEqual(logSpy.firstCall.args[0].context, testCase.expectedContext);
            assert.strictEqual(handlerCalled, true);
            sinon.restore();
            ClearcutLogger.resetForTesting();
        }
    ...
    it('reports unknown registered tool arguments clearly', async () => {
        ...
        assert.strictEqual(toolHandler.registeredInputSchema.safeParse(params).success, true);
        const result = await toolHandler.handle(params);
        assert.strictEqual(result.isError, true);
        assert.match(result.content[0].type === 'text' ? result.content[0].text : '', /Unknown argument for tool "lenient_tool": "description"\. Expected arguments: "url"\./);
    ...
    it('sets shouldRegister to false and returns disabled reason when category is disabled', async () => {
        ...
        const mockContext = sinon.createStubInstance(McpContext);
        const toolMutex = new Mutex();
        const serverArgs = parseArguments('1.0.0', ['node', 'script.js', '--categoryEmulation=false'], { CHROME_DEVTOOLS_MCP_NO_USAGE_STATISTICS: 'true' });
        const toolHandler = new ToolHandler(tool, serverArgs, async () => mockContext, toolMutex);
        assert.strictEqual(toolHandler.shouldRegister, false);
        const result = await toolHandler.handle({});
        assert.strictEqual(result.isError, true);
        assert.match(result.content[0].type === 'text' ? result.content[0].text : '', /is currently disabled/);
        assert.strictEqual(handlerCalled, false);
    });
    it('validates files specified in verifyFilesSchema and rewrites input with validated paths/URLs', async () => {
        ...
        mockContext.validatePath.callsFake(async (p) => {
            ...
            return path.resolve('/canonical', path.relative(path.resolve('/workspace'), p));
        });
        const toolMutex = new Mutex();
        const serverArgs = parseArguments('1.0.0', ['node', 'script.js'], {
            CHROME_DEVTOOLS_MCP_NO_USAGE_STATISTICS: 'true',
        ...
        assert.strictEqual(mockContext.validatePath.calledWith(testListFile1), true);
        assert.strictEqual(mockContext.validatePath.calledWith(testListFile2), true);
    ...
    it('returns error when file validation fails for verifyFilesSchema', async () => {
        ...
        const mockProcess = sinon.createStubInstance(ChildProcess);
        mockContext.browser = getMockBrowser({ process: mockProcess });
        mockContext.validatePath.rejects(new Error('Access denied: path is outside roots'));
        const toolMutex = new Mutex();
        const serverArgs = parseArguments('1.0.0', ['node', 'script.js'], {
            CHROME_DEVTOOLS_MCP_NO_USAGE_STATISTICS: 'true',
        });
        ...
        assert.strictEqual(result.isError, true);
        assert.match(result.content[0].type === 'text' ? result.content[0].text : '', /Access denied/);
        assert.strictEqual(handlerCalled, false);
    });
    ...
    it('validates verifyFilesSchema when local: true and browser is connected to localhost wsEndpoint', async () => {
        ...
        assert.strictEqual(result.isError, undefined);
        assert.strictEqual(handlerCalled, true);
        assert.strictEqual(mockContext.validatePath.calledOnceWith(bundlePath), true);
        assert.deepStrictEqual(receivedParams, {
    ...
    it('validates verifyFilesSchema with true but skips local: true on remote browser', async () => {
        ...
        assert.strictEqual(result.isError, undefined);
        assert.strictEqual(handlerCalled, true);
        assert.strictEqual(mockContext.validatePath.calledOnceWith(outputPath), true);
        assert.deepStrictEqual(receivedParams, {
    ...
    it('returns error when file validation fails for local: true on local browser', async () => {
        ...
        const mockProcess = sinon.createStubInstance(ChildProcess);
        mockContext.browser = getMockBrowser({ process: mockProcess });
        mockContext.validatePath.rejects(new Error('Path is outside configured roots'));
        const toolMutex = new Mutex();
        const serverArgs = parseArguments('1.0.0', ['node', 'script.js'], {
            CHROME_DEVTOOLS_MCP_NO_USAGE_STATISTICS: 'true',
        });
        ...
        assert.strictEqual(result.isError, true);
        assert.match(result.content[0].type === 'text' ? result.content[0].text : '', /Path is outside configured roots/);
        assert.strictEqual(handlerCalled, false);
    });
    it('validates remote: true on remote browser and skips on local browser', async () => {
        ...
        const remoteToolHandler = new ToolHandler(tool, serverArgs, async () => mockRemoteContext, toolMutex);
        const remotePath = path.resolve('/remote/file.txt');
        await remoteToolHandler.handle({ remoteFile: remotePath });
        assert.strictEqual(mockRemoteContext.validatePath.calledOnceWith(remotePath), true);
        ...
        const localToolHandler = new ToolHandler(tool, serverArgs, async () => mockLocalContext, toolMutex);
    ...
    it('rewrites file paths in params for page scoped tools', async () => {
        ...
        assert.strictEqual(result.isError, undefined);
        assert.strictEqual(mockContext.validatePath.calledOnceWith(inputPath), true);
        assert.deepStrictEqual(receivedParams, {
