...
describe('HeapSnapshotManager', () => {
    ...
    it('rejects when a file without .heapsnapshot or .heaptimeline extension is passed', async () => {
        ...
        await assert.rejects(manager.getSnapshot('tests/fixtures/snapshot_diffs.js'), /must have a \.heapsnapshot or \.heaptimeline extension/);
    });
    it('disposes the worker when snapshot loading fails', async () => {
        const disposeSpy = sinon.spy(DevTools.HeapSnapshotModel.HeapSnapshotProxy.HeapSnapshotWorkerProxy
            .prototype, 'dispose');
        const manager = new HeapSnapshotManager();
...
//# sourceMappingURL=HeapSnapshotManager.test.js.map