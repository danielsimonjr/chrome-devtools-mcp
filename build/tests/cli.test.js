...
describe('cli args parsing', () => {
    ...
    it('parses with browser url', async () => {
        const args = parseArguments('1.0.0', ['node', 'main.js', '--browserUrl', 'http://localhost:3000'], {});
        assert.deepStrictEqual(args, {
            ...defaultArgs,
            _: [],
            headless: false,
    ...
    it('parses with user data dir', async () => {
        const args = parseArguments('1.0.0', ['node', 'main.js', '--user-data-dir', '/tmp/chrome-profile'], {});
        assert.deepStrictEqual(args, {
            ...defaultArgs,
            _: [],
            headless: false,
    ...
    it('parses an empty browser url', async () => {
        const args = parseArguments('1.0.0', ['node', 'main.js', '--browserUrl', ''], {});
        assert.deepStrictEqual(args, {
            ...defaultArgs,
            _: [],
            headless: false,
    ...
    it('parses with executable path', async () => {
        const args = parseArguments('1.0.0', ['node', 'main.js', '--executablePath', '/tmp/test 123/chrome'], {});
        assert.deepStrictEqual(args, {
            ...defaultArgs,
            _: [],
            headless: false,
    ...
    it('parses viewport', async () => {
        const args = parseArguments('1.0.0', ['node', 'main.js', '--viewport', '888x777'], {});
        assert.deepStrictEqual(args, {
            ...defaultArgs,
            _: [],
            headless: false,
    ...
    it('parses disabled category', async () => {
        const args = parseArguments('1.0.0', ['node', 'main.js', '--no-category-emulation'], {});
        assert.deepStrictEqual(args, {
            ...defaultArgs,
            _: [],
            headless: false,
    ...
    it('parses usage statistics flag', async () => {
        ...
        // Test enabling it
        const enabledArgs = parseArguments('1.0.0', ['node', 'main.js', '--usage-statistics'], {});
        assert.strictEqual(enabledArgs.usageStatistics, true);
        // Test disabling it
        const disabledArgs = parseArguments('1.0.0', ['node', 'main.js', '--no-usage-statistics'], {});
    ...
    it('parses performance crux flag', async () => {
        ...
        // force enable
        const enabledArgs = parseArguments('1.0.0', ['node', 'main.js', '--performance-crux'], {});
        assert.strictEqual(enabledArgs.performanceCrux, true);
        const disabledArgs = parseArguments('1.0.0', ['node', 'main.js', '--no-performance-crux'], {});
    ...
    it('parses blocked-url-pattern flags as array', async () => {
        ...
        const singleArgs = parseArguments('1.0.0', ['node', 'main.js', '--blocked-url-pattern=https://example.com/*'], {});
        assert.deepStrictEqual(singleArgs.blockedUrlPattern, [
            'https://example.com/*',
        ]);
        const repeatedArgs = parseArguments('1.0.0', [
            'node',
            'main.js',
        ...
        const spaceSeparatedArgs = parseArguments('1.0.0', [
            'node',
            'main.js',
            '--blocked-url-pattern',
            'https://a.com/*',
    ...
    it('parses allowed-url-pattern flags as array', async () => {
        ...
        const singleArgs = parseArguments('1.0.0', ['node', 'main.js', '--allowed-url-pattern=https://example.com/*'], {});
