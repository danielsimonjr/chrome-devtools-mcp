...
describe('WaitForHelper', () => {
    ...
    it('does not stall when an action opens a dialog without handleDialog', async () => {
        await withMcpContext(async (response, context) => {
            ...
            try {
                ...
                assert(result !== 'stalled', 'stalled because a dialog was shown; would time out with ProtocolError');
    ...
    it('does not hang when an iframe navigates', async () => {
        await withMcpContext(async (response, context) => {
            ...
            const mcpPage = context.getSelectedMcpPage();
            await mcpPage.pptrPage.setContent(html `<iframe
          id="subframe"
          src="${iframeSrc}"
        ></iframe>`);
            ...
            assert(elapsed < 1500, `Took ${elapsed}ms; should not hang waiting for iframe`);
...
//# sourceMappingURL=WaitForHelper.test.js.map